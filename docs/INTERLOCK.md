...
This document is the single source of truth for Interlock: a runtime firewall that detects AI-agent data exfiltration across two observation planes (a userspace MCP proxy and a kernel eBPF sensor). Claims here are grounded in the implementation. Where older docs disagree with the code, this document states the code’s behavior and names the discrepancy.
...
Chose netns prevention in proxy mode over pretending the DaemonSet can do the same thing, because only the spawning parent can set `CLONE_NEWNET` before exec. That costs privilege (`CAP_SYS_ADMIN`) and breaks any legitimate outbound networking from sandboxed children (by design).

**Doc vs code:** some older README lines still imply a 512-byte default payload capture. Config and BPF default **`payload_capture_bytes` / `PAYLOAD_MAX` = 1024** (`internal/config`, `internal/ebpf/bpf/connect.c`).
...
Current path: on `decision.Action == model.ActionContained` (EXFIL only via `variantBAction`), handlers call `containPIDs` immediately, which calls `KillProcess` (SIGKILL to process group + pid). No timer, no deferred queue. Bare `connect()` never kills alone; soft SUSPICIOUS is `detected_only`.

**Doc vs code:** CHANGELOG / historical notes that still describe a deferred kill window are stale. `scripts/demo-k8s.sh` waits for demo delay + drain only; it does not assume a deferred-kill timer. This document is authoritative: immediate kill on EXFIL only.
...
**Doc vs code:** comments on `VerdictExfil` / `VerdictSuspicious` in `internal/model/model.go` still say “all legs + overlap” / “all legs, no overlap.” Trust `classifyTrip` literally:
...
EXFIL is overlap-alone. Soft legs can be fully decayed and EXFIL still fires if taint remains and overlap hits. AllLit gates SUSPICIOUS only.
...
**LSM deny** (`ingestLSMDeny`): Verdict=`EXFIL`, Action=`prevented`, Allow=false (repeat connect after quarantine).

**Doc vs code:** some architecture snippets described `ActionPrevented` as “Variant A only.” Model comments and LSM path also use `prevented` for repeat quarantine.
...
### Doc vs code: the “recency window”

`docs/architecture.md` §6 states that syscall events are joined to recent proxy events within a **recency window**. There is **no** such join window in `internal/engine/`. Correlation is: attribute PID/cgroup → `sessionID`, then evaluate against that session’s already-held legs and taints. Closest real time windows:
...
If you need a mental model: the session state *is* the join; it is not a sliding “events within N ms of each other” fuse.
...
## Appendix: doc vs code discrepancies (index)

| Topic | Stale claim | Code / this doc |
|---|---|---|
| Payload default | README ~512 in one place | Default **1024** = `PAYLOAD_MAX` |
| Recency window | architecture §6 proxy↔syscall join | No join window; PID/cgroup→session + session state; `SuspiciousConnectTTL` is write↔connect |
| Deferred kill | Historical ~100 ms | Symbols gone from `sensor.go`; immediate `containPIDs`→`KillProcess` on EXFIL only |
| EXFIL condition | Comments: AllLit + overlap | `if overlap != nil` returns EXFIL before `AllLit()` (`classifyTrip`) |
| Sensor soft SUSPICIOUS | Older prose implied always | Needs untrusted leg; bridge `register_untrusted` in sensor-only |
| `ActionPrevented` | “Variant A only” | Also LSM repeat quarantine |
| CO-RE | Docs emphasize CO-RE reads | `#include bpf_core_read.h` present; zero `BPF_CORE_READ` uses; `bpf_probe_read_user` only |
| Proxy `ts_mono_ns` | “shared monotonic” / Go mono | `time.Now().UnixNano()` (wall); BPF uses `bpf_ktime_get_ns()` (boot-relative) |
| ZIP/xlsx / git | Some CHANGELOG leftovers “open” | Containers largely closed §18/§20; git wire Named §21 |

*(Repo breadcrumb: historical references to `docs/SUMMARY.md` point nowhere; that file was removed. Gaps live in this document’s §16 and in `docs/architecture.md` §13 / ROADMAP.)*
