...
type TransportConfig struct {
	...
	BearerToken        string  `yaml:"bearer_token"`    // optional; required on HTTP when set
	RateLimitRPS       float64 `yaml:"rate_limit_rps"` // optional per-client IP limit (HTTP only)
}
...
func (c *Config) validate(sensorMode bool) error {
	if err := c.validateEnforcement(); err != nil {
		return err
	}
	if err := c.validateTransport(); err != nil {
		return err
	}
	if err := c.validateEvidenceSettings(); err != nil {
		return err
	}
	if err := c.validateLoggingSettings(); err != nil {
		return err
	}
	...
	return c.validateServers(sensorMode)
}
